...
from storage_stuff import PersistenceABC
...
# hmm actually i can just write a parser that generates tuples from a script.
article_script = """
system
You are a roleplayer, acting as a human.

assistant
I can do that. Any personality traits you want me to have?

system
Become the personification of this piece of text:
...
assistant
Hey, thanks for reading my article. You like it?
...
def parse_script(script):
    """Given a script, will return a list of tuples."""
    
    lines = script.split('\n')
    blocks = []
    this_block = []
    # first make blocks, then seperate each block into the role and content
    for l in lines:
        if l in ['system', 'assistant', 'user']:
            ...
            this_block.append(l)
    blocks.append(this_block)
    blocks = blocks[1:]  # remove the first empty block

    # now seperate each block into the role and content
    tuples = []
    for b in blocks:
        role = b[0]
        content = '\n'.join(b[1:])
...
# uses my PersistenceABC to save and load chatbot objects
class ChatbotStorage(PersistenceABC):
    """Handles saving and loading of a chatbot.
    Each bot has its own filename and storage object.
    
    abstract methods that the ABC requires:
        - implement_filedata
        - object_to_filedata

    the ABC yields save() and load() methods.
    """
    ...
    def object_to_filedata(self):
        """Just returns chatbot_object.messages"""

        # lod = list of dictionaries
        lod = self.chatbot_object.messages
        # check correctness
        assert type(lod) is list
        for item in lod:
            assert type(item) is dict
...
class Chatbot:
    ...
    def use_api(self):
        ...
        if self.messages == []:
            print('no context, returning None')
            return None

        api_response = use_chatgpt(self.messages)
        return api_response
...
cb_manager = ChatbotManager()
cb_manager.add_bot('jemma')
jemma = cb_manager.find_bot('jemma')
jemma.add_message(
    'assistant',
    '''Yes, I can certainly do that. Here's a quick summary of our conversation so far:

"Hello! As an AI language model, I introduced myself and offered to assist the user in any way possible. The user asked me to roleplay as a female AI like Jarvis and to tell them something interesting. I shared a fact about the immortal jellyfish and the user expressed their appreciation. Finally, the user requested that I occasionally summarize our conversation due to my limited context window."

I hope that helps!'''
)

while True:
    commands = ['show', 'talk', 'summarize']
    print(f'commands: {commands}')
    command = input('command: ')
    if command == 'show':
        print(jemma.get_messages_as_string())
    elif command == 'talk':
        message = input('message: ')
        jemma.talk_to_bot('user', message)
    elif command == 'summarize':
        cb_manager.create_successor(jemma)
        jemma = cb_manager.find_bot('jemma')
    else:
