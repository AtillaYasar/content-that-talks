...
# custom modules
from yttest import get_captions
...
def on_command(event):
    ...
    if 'youtube.com' in cmd:
        captions = get_captions(cmd)
        initial_text_widget.delete('1.0', 'end')
        initial_text_widget.insert('1.0', captions)
    elif cmd in commands:
        commands[cmd]()

# command to add a template message to the context widget
def add_message():
    # get context from context widget
    context = context_widget.get('1.0', 'end')
    messages = json.loads(context)
    ...
    messages.append({'role':'user/assistant', 'text':'template message'})
...
# command to delete the last message from the context widget
def delete_message():
    # get context from context widget
    context = context_widget.get('1.0', 'end')
    messages = json.loads(context)
